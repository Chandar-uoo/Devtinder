...
const Requests = () => {
  ...
  const reviewRequest = async (status, _id) => {
    try {
      const res = await axios.post(
        BaseURL + "/connectionRequest/reveiw" + "/" + status + "/" + _id,
        {},
  ...
  const fetchRequests = async () => {
    try {
      const requests = await axios.get(BaseURL + "/user/request/receveid", {
        withCredentials: true,
      });
      console.log(requests.data.pending);
      
      dispatch(addrequests(requests.data.pending));
